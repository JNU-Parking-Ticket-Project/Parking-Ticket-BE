...
public class NoticeAdaptor implements NoticeRecordPort, NoticeLoadPort {
    ...
    /**
     * 안내사항을 조회한다. 안내사항이 존재하지 않는 경우 404 Error를 반환한다.
     *
     * @return Notice
     */
    @Override
    public Notice findLastOne() {
        return noticeRepository
                .findFirstByOrderByNoticeIdDesc()
                .orElseThrow(() -> NoticeNotExistException.EXCEPTION);
    }

    /**
     * 안내사항을 업데이트한다. 안내사항이 존재하지 않는 경우 작성된 안내사항을 저장 후 해당 안내사항을 반환한다.
     *
    ...
    public Notice updateNotice(String noticeContent) {
        ...
        if (notice.isPresent()) {
            ...
            return noticeRepository.save(Notice.builder().noticeContent(noticeContent).build());
        }
    }
}
