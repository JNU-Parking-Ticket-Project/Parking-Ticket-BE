...
5. `V5__add_registration_admission_journal.sql`: 이벤트 admission mode/epoch와 신청 결정 저널
...
3. 기존 스키마가 V1 기준 테이블을 보유했는지 확인하고 아래 V4/V5 사전 점검을 실행합니다.
...
6. `flyway_schema_history`에 version 1 baseline과 version 2, 3, 4, 5 success가 기록됐는지 확인합니다.
7. `registration_tb` 결과 컬럼, `email_outbox`, 신청 결정 저널, 관련 인덱스와 과거 결과 검증 쿼리를 확인합니다.
...
V5는 `event`에 `admission_mode`, `admission_epoch`, `version`을 추가하고 `registration_admission_journal`을 생성합니다. 기존 이벤트 행은 `REDIS`, epoch `0`, version `0`으로 시작합니다.

이 스키마는 Redis 정상 경로의 비동기 본 저장과 Redis 장애 시 DB fallback 사이의 결정 경계입니다. 애플리케이션은 다음 상태를 저널에 보존합니다.

- `RECEIVED`: 최종 신청 요청과 복구 payload를 DB에 기록함
- `DECIDED`: Redis 또는 DB가 position과 합격·예비 결과를 확정함
- `MATERIALIZED`: Registration, User, Sector, Email Outbox 본 저장을 완료함
- `REJECTED`: 재고 소진 등으로 접수를 거절함

### scale-up / scale-down 계약

- 이벤트 시작 전 scale-up이 끝나고 Redis가 준비된 뒤에만 이벤트를 OPEN합니다.
- scale-up 인스턴스에서 이벤트 운영 중 Redis가 실패하면 이벤트 mode를 영속적인 `DB_FALLBACK`으로 전환하고 신청을 계속 받습니다. 이 경로는 본 저장 DB commit 후 응답합니다.
- `DB_FALLBACK`으로 전환된 이벤트는 같은 이벤트 동안 Redis mode로 자동 복귀하지 않습니다.
- scale-down처럼 Redis bean이 없는 구성에서는 임시 저장만 사용할 수 있습니다. 신규 최종 신청과 이벤트 OPEN은 fail-closed입니다.
- 이미 확정된 동일 요청의 응답 유실 재시도는 기존 저널 결과를 재생할 수 있으며 신규 신청으로 보지 않습니다.

### 배포 전 필수 조건

V5와 새 애플리케이션은 이벤트가 OPEN인 동안 rolling deployment로 섞어 실행하면 안 됩니다. 구버전은 결정 저널과 admission epoch를 사용하지 않아 같은 이벤트의 신청 결정을 안전하게 공유할 수 없습니다.

1. 이벤트가 `OPEN` 또는 `CALCULATING`이 아닌지 확인합니다.
2. 레거시 Stream 메시지가 실제 Registration과 Outbox로 모두 반영됐는지 확인합니다.
3. 모든 consumer group의 Stream lag와 pending이 `0`인지 확인합니다.
4. 각 애플리케이션 인스턴스의 local in-flight 처리가 `0`인지 확인합니다.
5. 구버전 인스턴스를 신청 경로에서 모두 제거합니다.
6. V5 migration과 Hibernate validation을 통과한 새 버전만 기동합니다.
7. Redis ping, initialized marker, 구간별 stock/sequence를 확인한 뒤 다음 이벤트를 OPEN합니다.

스택은 PR 순서대로 병합하되, 운영 release는 V5와 최종 admission 코드를 포함한 버전으로 한 번에 진행합니다. 저널이 없는 중간 스택 버전을 운영에 자동 배포하지 않습니다.

이미 V4까지 적용된 DB에서는 V5 한 건만 실행됩니다. V5 적용 전후에 다음 스키마를 확인합니다.

```sql
SELECT event_id, event_status, admission_mode, admission_epoch, version
FROM event
ORDER BY event_id;

SHOW CREATE TABLE registration_admission_journal;

SELECT state, COUNT(*)
FROM registration_admission_journal
GROUP BY state;
```

배포 직후 저널이 비어 있는 것은 정상입니다. 다음 최종 신청부터 행이 생성됩니다.
...
version 4와 5가 `success = 1`, 누락 결과가 `0`, 마지막 쿼리가 0행인지 확인합니다.
...
- 애플리케이션만 이전 버전으로 되돌려야 할 때는 이전 버전이 추가 컬럼과 테이블을 무시할 수 있는지 먼저 확인합니다. 이벤트 OPEN 중에는 저널을 모르는 구버전으로 rollback하지 않습니다.
