...
public record FindPasswordResponse(String message) {
    ...
    public static FindPasswordResponse of(boolean isSend) {
        if (isSend) {
            return FindPasswordResponse.builder().message(MailMessage.SUCCESS_SEND_MAIL).build();
        }
        return FindPasswordResponse.builder().message(MailMessage.FAIL_SEND_MAIL).build();
    }
}
