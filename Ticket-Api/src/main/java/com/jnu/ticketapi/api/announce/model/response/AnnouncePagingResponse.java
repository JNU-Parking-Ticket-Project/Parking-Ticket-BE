...
public record AnnouncePagingResponse(
    ...
    public static AnnouncePagingResponse of(Page<Announce> announcePaging) {
        if (!announcePaging.hasNext()) {
            return AnnouncePagingResponse.newLastScroll(announcePaging.getContent(), announcePaging.getTotalPages() - 1);
        }
        return AnnouncePagingResponse.newPagingHasNext(announcePaging.getContent(), announcePaging.getTotalPages() - 1, announcePaging.getPageable().getPageNumber() + 1);
    }

    private static AnnouncePagingResponse newLastScroll(List<Announce> announcePaging, long lastPage) {
        return newPagingHasNext(announcePaging, lastPage, LAST_PAGE);
    }

    private static AnnouncePagingResponse newPagingHasNext(List<Announce> announcePaging, long lastPage, long nextPage) {
    ...
    private static List<AnnounceInfoResponse> getAnnounces(List<Announce> announcePaging) {
        return announcePaging.stream()
                .map(AnnounceInfoResponse::of)
