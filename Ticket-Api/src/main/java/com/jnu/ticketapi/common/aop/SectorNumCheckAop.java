...
public class SectorNumCheckAop {
    ...
    //                        checkDuplication((SectorRegisterRequest) request,
    // uniqueSectorNumbers);
    //                    }
    //                }
    //            }
    ...
    public void checkSectorNumberDuplication(JoinPoint joinPoint) {
        Object[] args = joinPoint.getArgs();
        List<SectorRegisterRequest> requests =
                Arrays.stream(args)
                        ...
                        .toList();
        List<String> distinctRequest =
                requests.stream().map(SectorRegisterRequest::sectorNumber).distinct().toList();
