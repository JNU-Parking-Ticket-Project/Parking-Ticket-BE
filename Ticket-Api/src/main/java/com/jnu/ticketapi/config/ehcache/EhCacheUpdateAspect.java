...
public class EhCacheUpdateAspect {
    ...
    public void updateCache(JoinPoint joinPoint, CacheUpdate cacheUpdate, Object result) {
        ...
        if (cache == null) {
            throw new IllegalArgumentException("Cache " + cacheUpdate.cacheName() + " 가 존재하지 않습니다."); //TODO 추후 커스텀 양식대로 수정
        }
        AnnounceDetailsResponse oldAnnounceDetails = cache.get(cacheUpdate.key(), AnnounceDetailsResponse.class); // 기존 캐시 데이터
        ...
        if (result instanceof UpdateAnnounceResponse updateAnnounceResponse) {
            ...
            AnnounceDetailsResponse newAnnounceDetails = convertToAnnounceDetailsResponse(updateAnnounceResponse, announceId, oldAnnounceDetails.announceCreatedAt());
    ...
    private AnnounceDetailsResponse convertToAnnounceDetailsResponse(UpdateAnnounceResponse updateAnnounceResponse, Long announceId, LocalDateTime createdAt) {
